...
all_the_stuff <- function(burden, mutation_rate) {
  ...
  times <- seq(0,100, 0.1)    # time sequence for ODE solver
...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    radioButtons("volumes", label = "Generations to Saturate Volumes:",
  ...
  output$produce <- renderPlot({
    ...
    ggplot(data = out.here, 
      ...
      labs(title = "Production Curve", 
           x = "Cell Doublings", 
           y = "Fraction of Engineered Cells") +
      NULL
  }, 
  height = 600)
}
